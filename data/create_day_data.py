...
def main():
    ...
    for day in days:
        ...
        # for x in listdir:
        #     basename = os.path.splitext(x[1:])[0]
        #     if basename in videos:
        #         os.rename('../img/' + day + '/' + x, '../img/' + day + '/' + x[1:])

        for x in listdir:
            if x.startswith('.') or x.startswith('_') or os.path.splitext(x)[0] in videos: continue # underscores indicate thumb photos
            ...
            if media['filetype'] == 'photo':
                # media['path'] = 'img/day' + str(day_num_UI) + '/' + x
                media['thumb'] = 'img/day' + str(day_num_UI) + '/_thumb_' + x
            elif media['filetype'] == 'video':
                # media['path'] = '<iframe src="' + video_map[x] + '" width="640" height="480"></iframe>'
                media['thumb'] = 'img/day' + str(day_num_UI) + '/_thumb_' + x + '.jpg'
